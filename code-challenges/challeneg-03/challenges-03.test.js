...
Write a function named sortMeetingsByDay that takes in an array of objects, each of which represents a meeting happening
 a particular day of the week, with a particular start time and end time.

Sort the meetings by the day on which they happen, Monday-Friday. It does not matter which order meetings come in on
 a particular day. For example, if there are two meetings on Monday, it does not matter which comes first.
------------------------------------------------------------------------------------------------ */
...
const sortMeetingsByDay = (arr) => {
  //modified from https://stackoverflow.com/questions/36083827/sorting-object-sunday-to-saturday-in-javascript
  const order= {
    Monday: 1,
    Tuesday: 2,
    Wednesday: 3,
...
Sort the meetings in the order that they start. If two meetings start at the same time on the same day, the shorter 
meeting should come first.
...
------------------------------------------------------------------------------------------------ */
  ...
  const sortSchedule = (arr) => {
    ...
    return sortedByDay.sort((a, b) => {
      if(a.dayOfWeek === b.dayOfWeek){         //If two meetings  on the same day,
        if(a.start === b.start){              //If two meetings start at the same time,
          return a.end - b.end;               //the shorter meeting should come first.
        }
